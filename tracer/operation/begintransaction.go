...
type BeginTransaction struct {
	TransactionNumber uint32 // txcontext number
}

// GetId returns the begin-txcontext operation identifier.
func (op *BeginTransaction) GetId() byte {
	return BeginTransactionID
}

// NewBeginTransaction creates a new begin-txcontext operation.
func NewBeginTransaction(tx uint32) *BeginTransaction {
	return &BeginTransaction{tx}
}

// ReadBeginTransaction reads a new begin-txcontext operation from file.
...
// Debug prints a debug message for the begin-txcontext operation.
