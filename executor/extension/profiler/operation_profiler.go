...
func MakeOperationProfiler[T any](cfg *utils.Config) executor.Extension[T] {
	...
	p.ps.AddPrintToSqlite3(p.sqlite3(cfg.ProfileSqlite3))
...
func (p *operationProfiler[T]) sqlite3(conn string) (string, string, func() [][]any) {
	insert := "INSERT INTO operations(start, end, opId, opName, count, sum, mean, std, variance, skewness, kurtosis, min, max) VALUES( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )"
