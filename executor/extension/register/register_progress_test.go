...
func TestRegisterProgress_InsertToDbIfEnabled(t *testing.T) {
	...
	if _, err := ext.(extension.NilExtension[substate_transaction.SubstateData]); err {
	...
	sub := substate_transaction.NewOldSubstateData(s)

	for b := int(cfg.First); b < int(cfg.Last); b++ {
		ext.PreBlock(executor.State[substate_transaction.SubstateData]{Block: b, Data: sub}, ctx)
		...
		ext.PreTransaction(executor.State[substate_transaction.SubstateData]{Data: sub}, ctx)
		ext.PostTransaction(executor.State[substate_transaction.SubstateData]{Data: sub}, ctx)
		ext.PostBlock(executor.State[substate_transaction.SubstateData]{Block: b, Data: sub}, ctx)
	}

	ext.PostRun(executor.State[substate_transaction.SubstateData]{}, ctx, nil)
...
func TestRegisterProgress_IfErrorRecordIntoMetadata(t *testing.T) {
	...
	ext.PreRun(executor.State[substate_transaction.SubstateData]{}, ctx)
	ext.PostRun(executor.State[substate_transaction.SubstateData]{}, ctx, fmt.Errorf(errorText))
