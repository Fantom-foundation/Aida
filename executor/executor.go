...
type Params struct {
	...
	// value of 0 is valid.
	NumWorkers int
	// ParallelismGranularity determines whether parallelism is done on block or txcontext level
...
type Processor[T any] interface {
	// Process is called on each txcontext in the range of blocks covered
...
type Extension[T any] interface {
	...
	// it references the last txcontext attempted to be processed. Also,
	...
	// the state containing the number of the Block and the last txcontext
	// processed in the block. This function is not called when running with
	// multiple workers.
	PostBlock(State[T], *Context) error

	// PreTransaction is called once before each txcontext with the state
	// listing the block number, the txcontext number, and the substate data
	// providing the input for the subsequent execution of the txcontext.
	...
	// PostTransaction is called once after each txcontext with the state
	// listing the block number, the txcontext number, and the substate data
	// providing the input for the subsequent execution of the txcontext.
	// When running with multiple workers, this function may be called
	// concurrently, and must thus be thread safe.
...
type State[T any] struct {
	...
	// Transaction is the txcontext number of the current txcontext within
	...
	// Data is the input required for processing the current txcontext. It is
...
// runBlock runs txcontext execution in a block
func runBlock[T any](
	workerNumber int,
