...
success <- runSucceed$value == "true"
```

**Run `r runId$value`** `r if(success){
	paste0("completed successfully")
} else {
	paste0("failed")
...
Unit <- c("Gb", "Gb", "Gb", "TPS", "MGPS")
Conv <- c(1e-9, 1e-9, 1e-9, 1, 1e-6)
...
The average gas rate is **`r format(mean(TxData$gas_rate) * 1e-6, big.mark=",", digit=4)`** MGPS. The max gas rate  is **`r format(maxGasRate$gas_rate * 1e-6, big.mark=",", digit=4)`** MGPS at block height **`r format(maxGasRate$start, big.mark=",")`**. 
...
TxData %>% 
   ...
   geom_point(alpha=0.3) +
   scale_x_continuous(labels = scales::unit_format(unit="M")) +
   scale_y_continuous(labels = scales::unit_format(unit="MGPS")) +
