...
The experiment is run on the machine **`r hostname$value`**(**`r ip$value`**), which is a **`r processor$value`**, **`r memory$value`**.
...
```{r, echo=FALSE, message=FALSE}
maxMemory <- TxData %>% filter(memory == max(memory)) %>% distinct(memory) 
maxLiveDisk <- TxData %>% filter(live_disk == max(live_disk)) %>% distinct(live_disk) 
maxArchiveDisk <- TxData %>% filter(archive_disk == max(archive_disk)) %>% distinct(archive_disk) 
```
...
```{r, echo=FALSE, message=FALSE}
maxTxRate <- TxData %>% filter(tx_rate == max(tx_rate)) %>% distinct(tx_rate)
```
...
```{r, echo=FALSE, message=FALSE}
maxGasRate <- TxData %>% filter(gas_rate == max(gas_rate)) %>% distinct(gas_rate) 
```
On average, the gas rate is **`r format(mean(TxData$gas_rate), big.mark=",")`** GPS. The max gas rate  is **`r format(maxGasRate$gas_rate, big.mark=",")`** GPS at block height **`r format(maxGasRate$start, big.mark=",")`**. 
