...
func LoadAccounts(db state.Database, root common.Hash, workers int) (chan types.Account, chan error) {
	...
	go loadAccounts(db, root, out, err, workers)
	return out, err
}

// iterate the state proxying account state assembly to workers.
func loadAccounts(db state.Database, root common.Hash, out chan types.Account, fail chan error, workers int) {
	defer func() {
		close(out)
		close(fail)
	}()
	...
	raw, rawErr := LoadRawAccounts(db, root)

	// start individual workers to extend accounts with code and storage and wait for them to finish
	var wg sync.WaitGroup
	for i := 0; i < workers; i++ {
		wg.Add(1)
		go finaliseAccounts(db, raw, out, fail, &wg)
	}
	wg.Wait()

	// do we have a raw accounts read error? copy the error to error output
...
func LoadRawAccounts(db state.Database, root common.Hash) (chan types.Account, chan error) {
	assembly := make(chan types.Account, 25)
	err := make(chan error, 1)

	go loadRawAccounts(db, root, assembly, err)
	return assembly, err
}

// loadRawAccounts iterates over evm state then sends individual accounts to inAccounts channel
func loadRawAccounts(db state.Database, root common.Hash, raw chan types.Account, fail chan error) {
	defer func() {
		close(raw)
		close(fail)
	}()
...
func finaliseAccounts(db state.Database, in chan types.Account, out chan types.Account, fail chan error, wg *sync.WaitGroup) {
	defer func() {
		wg.Done()
	}()
...
func assembleAccount(db state.Database, acc *types.Account) error {
	var err error
	...
	if acc.Root != eth.EmptyRootHash {
		acc.Storage, err = loadStorage(db, acc)
		if err != nil {
...
func loadStorage(db state.Database, acc *types.Account) (map[common.Hash]common.Hash, error) {
