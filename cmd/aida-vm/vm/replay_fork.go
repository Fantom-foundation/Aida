...
var ReplayForkCommand = cli.Command{
	...
	Description: `
The vm-fork command requires two arguments:
...
func hardForkFlagDefault() int64 {
	...
	if v <= 0 {
		panic(fmt.Errorf("aida-vm replay-fork: corrupted --hard-fork default value: %v", v))
	}
	return v
}
...
// record-vm: func replayForkAction for replay-fork command
func replayForkAction(ctx *cli.Context) error {
	...
	if hardForkName, exist := HardForkName[hardFork]; !exist {
		return fmt.Errorf("aida-vm replay-fork: invalid hard-fork block number %v", hardFork)
	} else {
	...
	taskPool := substate.NewSubstateTaskPool("aida-vm replay-fork", replayForkTask, cfg.First, cfg.Last, ctx)
