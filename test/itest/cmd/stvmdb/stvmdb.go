...
func stVmDb(ctx *cli.Context) error {
	...
	db, stateDbDir, err := PrepareStateDB(cfg)
	if err != nil {
		return err
	}
	if !cfg.KeepDb {
	...
	for iter.Next() {
		...
		err = ProcessTx(db, cfg, tx.Block, tx.Transaction, tx.Substate)
		if err != nil {
			log.Critical("\tFAILED")
	...
	if cfg.ContinueOnFailure {
		log.Warningf("%v errors found", NumErrors)
	}
	...
	*/
	...
	if cfg.KeepDb && !isFirstBlock {
		...
		if err := WriteStateDbInfo(stateDbDir, cfg, curBlock, rootHash); err != nil {
			log.Error(err)
		}
		//rename directory after closing db.
		defer RenameTempStateDBDirectory(cfg, stateDbDir, curBlock)
	...
	if !cfg.Quiet {
		...
		log.Infof("Final disk usage: %v MiB\n", float32(GetDirectorySize(stateDbDir))/float32(1024*1024))
	}
